...
editItemWidget*editItemWidgetFactory::getNewCurrencyWidget(QWidget* parent)
{
	...
	QVector<bool> btnEnableList;
	btnEnableList << false << true << true;
	auto ans = buildWidget(parent, "新建货币", labnamelist, labvisiblelist, "", "", true, true, btnEnableList);
...
editItemWidget*editItemWidgetFactory::getNewCategoryWidget(QWidget* parent)
{
	...
	QVector<bool> btnEnableList;
	btnEnableList << false << true << true;
	auto ans = buildWidget(parent, "新建种类", labnamelist, labvisiblelist, "", "", false, true, btnEnableList);
...
editItemWidget*editItemWidgetFactory::getEditCategoryWidget(QWidget* parent, const QString& cateName)
{
	...
	QVector<bool> btnEnableList;
	btnEnableList << false << true << true;
	auto ans = buildWidget(parent, "编辑种类", labnamelist, labvisiblelist, "", "", false, true, btnEnableList);
...
editItemWidget*editItemWidgetFactory::getEditCurrencyWidget(QWidget* parent, const QString& currName)
{
	...
	QVector<bool> btnEnableList;
	btnEnableList << true << true << true;
	auto ans = buildWidget(parent, "编辑货币", labnamelist, labvisiblelist, "", "", true, true, btnEnableList);
...
editItemWidget* editItemWidgetFactory::buildWidget(QWidget* parent, const QString& title, \
										const QStringList& labnamelist, const QVector<bool>& labvisiblelist,\
										...
										const QVector<bool>& btnEnableList)
{
	...
	result->setLineEdit2(defaultText2, line2visible);
	result->setLineEdit3(defaultText3, line3visible);
	result->setConformBtn(btnEnableList[0]);
	result->setDeleteBtn(btnEnableList[1]);
	result->setCancelBtn(btnEnableList[2]);
	return result;
}
